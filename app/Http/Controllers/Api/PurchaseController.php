<?php

namespace App\Http\Controllers\Api;

use Carbon\Carbon;
use App\Models\User;
use App\Models\Products;
use App\Models\Purchase;
use App\Models\UserAmount;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\Auth;

class PurchaseController extends Controller
{
    public function index($id)
    {
        $product = Products::find($id);
        
        if (!$product) {
            return response()->json([
                'status' => 404,
                'message' => "Product Not Found."
            ]);
        }
        
        $now = Carbon::now();
        $user = Auth::user();

        if (!Auth::user()->identity) {
            return response()->json([
                'status' => 200,
                'message' => "Upload Your Identity Card First."
            ]);
        }

        if (!$user->amount) {
            return response()->json([
                'status' => 200,
                'message' => "You have no capital balance. Please top-up first !"
            ]);
        }

        if ($user->amount->capital < $product->price) {
            return response()->json([
                'status' => 200,
                'message' => "Your balance is insufficient!"
            ]);
        }

        $check = Purchase::where('user_id', Auth::id())
                         ->where('product_id', $id)
                         ->whereDate('created_at', $now)
                         ->count();
        
        if ($check) {
            return response()->json([
                'status' => 200,
                'message' => "You can not buy this product yet!"
            ]);
        }
        
        $today_purchases = Purchase::where('user_id', Auth::id())
                                  ->whereDate('created_at', $now)
                                  ->count();
        
        if ($today_purchases >= 20) {
            return response()->json([
                'status' => 200,
                'message' => "Maximum limit order per day is 20 !"
            ]);
        }

        $balance = Purchase::whereDate('created_at', $now)
                            ->where('user_id', Auth::id())
                            ->get()
                            ->sum('price');
                
        if (($balance + $product->price) > $user->amount->capital) {
            return response()->json([
                'status' => 200,
                'message' => "You have exceeded your limit!"
            ]);
        }

        $commission = $product->price * 0.05 ;

        $purchase = new Purchase();
        $purchase->product_id = $id;
        $purchase->user_id = Auth::id();
        $purchase->price = $product->price;
        $purchase->commission = $commission;
        $purchase->save();

        UserAmount::calculate_click_commission($user->id, $commission);
        UserAmount::calculate_level_commission($purchase, $commission);
        return response()->json([
            'status' => 200,
            'message' => "Product purchase successfully"
        ]);
    }
}
